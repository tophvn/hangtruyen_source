            ...
            <div class="card-header">
                ...
                <div class="card-tools">
                    <form method="GET" action="{{ route('admin.comments') }}" class="d-flex gap-2">
                        <input type="text" name="search" class="form-control form-control-sm" placeholder="Tìm theo nội dung..." value="{{ $search }}" style="width: 200px;">
                        <input type="text" name="user_search" class="form-control form-control-sm" placeholder="Tìm theo user..." value="{{ $userSearch }}" style="width: 200px;">
                    ...
                    <tbody>
                        @forelse($comments as $comment)
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($comment->manga)
                                        <a href="{{ route('manga.detail', ['slug' => $comment->manga->slug]) }}" target="_blank" class="text-decoration-none">
                                ...
                                <td>
                                    @if($comment->chapter && !empty($comment->chapter->slug))
                                        <a href="{{ route('manga.chapter', ['mangaSlug' => $comment->manga->slug ?? '', 'chapterSlug' => $comment->chapter->slug]) }}" target="_blank" class="text-decoration-none">
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-danger delete-comment" data-comment-id="{{ $comment->id }}" data-comment-content="{{ Str::limit($comment->content, 50) }}">
                ...
                <div class="card-footer">
                    ...
                    @if($comments->total() > 20)
                        <div class="text-muted text-center mt-2">
                            Hiển thị {{ $comments->firstItem() }} - {{ $comments->lastItem() }} trong tổng số {{ $comments->total() }} bình luận
                        </div>
                    @endif
                </div>
            @endif
